...
const Register = () => {
  const [emailCofirm, setEmailConfirm] = useState(false);
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setEmailConfirm(res.data.email);
  ...
  return (
    <>
      {emailCofirm ? (
        <FormOtp email={emailCofirm} />
      ) : (
        <section className="w-full  flex justify-center">
          <div className="bg-[#689F38] lg:w-1/3 w-4/5 md:w-2/5 mt-28 rounded py-5">
            <h2 className="font-bold text-center text-white text-xl">
              REGISTER
            </h2>
